...
public:
	...
	void draw(const char* text, int textLength, glm::vec2 pos, float size, float space, glm::vec4 color) {
	...
	~Text() {}
private:
	Shader shader{ "GUI.vert", "GUI.frag", nullptr, false };
	Mesh mesh;
	Buffer buffer{ GL_ARRAY_BUFFER, 256 * sizeof(GLfloat), nullptr, GL_STATIC_DRAW };
	Texture font{ "fontSDF.png", GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, true };
};
